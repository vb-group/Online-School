...
class WLSM_Menu {
	...
	public static function create_menu() {
		if ( WLSM_Helper::lm_valid() ) {
			$school_management = add_menu_page( esc_html__( 'School Management', 'school-management' ), esc_html__( 'All Management', 'school-management' ), WLSM_ADMIN_CAPABILITY, WLSM_MENU_SM, array( 'WLSM_Menu', 'school_management' ), 'dashicons-welcome-learn-more', 27 );
			...
			if ( $current_school = $user_info['current_school'] ) {
				...
				if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
					...
					// School - Menu.
					$school_staff_school_menu = add_menu_page( esc_html__( 'School', 'school-management' ), esc_html__( 'VB School', 'school-management' ), 'read', WLSM_MENU_STAFF_SCHOOL, array( 'WLSM_Menu', 'school_staff_dashboard' ), 'dashicons-building', 31 );
