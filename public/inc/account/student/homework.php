...
$page_url = WLSM_M_Staff_Class::get_homeworks_submisson_page_url();
...
$homeworks_student = $wpdb->get_results($wpdb->prepare('SELECT ID, student_id, created_at, description FROM ' .WLSM_HOMEWORK_SUBMISSION.' as hs WHERE hs.student_id = '.$student_id.' ORDER BY hs.ID DESC '));
				...
				<table class="table table-bordered wlsm-student-payment-history-table wlsm-w-100">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<a class="btn btn-primary btn-sm ml-2" id="homework_submission_edit" href="<?php echo esc_url(($page_url).'school-student-login/?action=submit-homework&id='.($row->ID)) ?>" ><?php esc_html_e( 'Edit', 'school-management' ); ?></a>
							</td>
						</tr>
						<?php
						}
