...
if ( isset( $_GET['id'] ) && ! empty( $_GET['id'] ) ) {
	...
	$homeworks = $wpdb->get_results($wpdb->prepare('SELECT hw.ID, hs.attachments, hw.title, hs.student_id, hs.created_at, hs.description FROM ' .WLSM_HOMEWORK_SUBMISSION.' as hs
	...
	<div class="wlsm-st-leave-request-section">
		<form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post" id="wlsm-submit-student-homework-submission-form">
			...
			<div class="wlsm-form-group">
				...
				<textarea required name="description" class="wlsm-font-control" id="wlsm_description" placeholder="<?php esc_attr_e('Enter here', 'school-management'); ?>" cols="30" rows="4" > <?php if ( $homework->description && $id) { echo "$homework->description"; } ?></textarea>
